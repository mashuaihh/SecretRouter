 ...
 *
 */
public class TestroutingEFRPath {

	/**
	 * 
	 */
	public TestroutingEFRPath(DistributionResource resDistribution) {
	 ...
	 * for testing ERF
	 */
	public void TestEFRshortestPath(){
    	// create edges
	    ...
	    long times = 5;
	        for(routerNode v : vertices) {
	            for(routerNode w : vertices) {
	                if (v.equals(w) == false) // don't include self-distances
	                {
	                	simulationEvent se = new simulationEvent(5, 0, v, DResource.getrQueue()[2], w);
	                	routing(se);
	...
	public void routing(simulationEvent se){
		...
		if(se.getEventNode()==se.getResourceNode()){
        	...
        	System.out.println("Shortest" + se.getEventNode().getid()+ "-" + se.getResourceNode().getid() +":");
        	...
        	System.out.println("EFR" + se.getEventNode().getid()+ "-" + se.getResourceNode().getid() +":");
...
/*		int j;
		...
		System.out.println(rateStatistic.doubleValue());
	*/	
	}
