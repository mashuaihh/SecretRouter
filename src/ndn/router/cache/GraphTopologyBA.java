/**
 * @(#)GraphTopologyGenerator.java	V1 14/02/11
 *
 * @author LinSen
 * 
 */
...
public class GraphTopologyBA {
	 ...
	 */
    public GraphTopologyBA(){
    	// generate a new graph
    	if(routerMain.iswitch == 0){
    		// create a BA model
    		BarabasiAlbertGenerator<routerNode, routerLink> BAGraph =
    			new BarabasiAlbertGenerator<routerNode, routerLink>(
    		    	new GraphFactory(), new VertexFactory(), new EdgeFactory(), 
    		    	5, 3, new HashSet<routerNode>()); // initial vertex number:5 m =3;
  
    		// get the graph  
        	rgraph = BAGraph.create();
        	
        	// create edges. edge is added duplicately
    	    Collection<routerNode> vertices = rgraph.getVertices();
  	        for(routerNode v : vertices) {
   	            for(routerNode w : vertices) {
   	                if (v.equals(w) == false) // don't include self-distances
   	                {
   	                	if(!rgraph.addEdge(new routerLink(0 ,Ecount++, 1, 1), v, w))
   	                		;//System.out.println("edge added duplicately!");
    	            }
    	        }
  	        }
    			
            // generate BA graph
   			BAGraph.evolveGraph(routerMain.vertexNum - 5);  // total vertex number: 600
            ...
            try{
                PrintWriter graphout = new PrintWriter(new FileWriter("C:/GraphTopologyBA.txt"));
                // output vertices
                ...
                graphout.close();
                System.out.println("BA average degree:" + elist.size()*2.0/vlist.size());
            }catch(Exception e){
            	e.printStackTrace();
            }
        }
    	// read from the file
            ...
            try{
            	...
            	BufferedReader graphreader = new BufferedReader(new FileReader("C:/GraphTopologyBA.txt"));
                ...
                System.out.println("BA average degree:" + edgeNumber*2.0/verNumber);
          		// close the file
                graphreader.close();
            }catch(Exception e){
            	e.printStackTrace();
	 ...
	 */
	class GraphFactory implements Factory<Graph<routerNode,routerLink>> {
